...
> 多数量产乘用车仍以 L2/L2+ 辅助驾驶为主；少数车型在限定法规区域和设计运行范围内提供 L3 功能，L4 Robotaxi 则主要在特定城市区域示范或商业运营。
...
- **线控转向（Steer-by-Wire）**：真正线控方案中方向盘与车轮可无机械连接，由电机和冗余控制执行
...
| **英伟达 Thor** | NVIDIA | 1000+ TOPS 级（随版本/精度口径变化） | 先进制程 | 中央计算（新一代平台） |
| **地平线征程 5** | 地平线 | 128 TOPS | 16nm | 智驾域：国产方案/性价比 |
| **特斯拉 HW4.0** | Tesla | 未官方统一披露 | 定制芯片 | 中央计算：FSD 全栈 |
...
**原理（说人话）**：车机 OS 跟手机 OS 最大的区别是——手机死机大不了重启，车机死机可能影响导航/空调/驾驶模式。所以车机 OS 必须做到：(1) **启动快**（上车后秒级可用）；(2) **不卡不死**（-40°C 到 85°C 等车规环境下稳定运行）；(3) **安全隔离**（娱乐系统 ≠ 仪表系统，仪表那侧的 Linux/QNX 不能被中控娱乐问题拖垮）。
...
|------|----------|-----------|----------|------|------|
| **纯视觉** | 主要依赖摄像头和视觉算法理解环境 | 多摄像头（是否带雷达随车型/年份变化） | 特斯拉 | 成本低、系统简化 | 恶劣天气/强逆光/异形障碍物 |
...
实际工程取舍：
- 特斯拉 FSD（视觉为主）：依赖海量车队数据和端到端模型能力，但仍需要处理「幽灵刹车」、施工区和长尾障碍物等问题
- 华为 ADS 等多传感器方案：通过摄像头、毫米波雷达、激光雷达等提升冗余，复杂路口和遮挡场景更容易建立安全边界
- 趋势：量产方案会继续在成本、传感器冗余、数据规模和算法能力之间取平衡，激光雷达价格下降后，上车门槛明显降低

**油电对比 / 生活类比**：
- 油电对比：高阶智驾更集中出现在新能源和智能化车型上，因为它们的电子电气架构、算力和 OTA 能力更容易支撑持续迭代；燃油车也可以搭载 ADAS，但平台改造空间通常更受限制。
