...
> 高精地图曾是很多 L3+ / NOA 方案的重要基础设施，但无图化和轻地图路线也在发展。无论采用哪条路线，定位和地图数据都需要持续更新与失效降级。
...
**原理（说人话）**：功能安全（ISO 26262）回答"如果电子系统坏了怎么办"——比如制动 ECU 死机了，有没有备份系统接管？ASIL D 不是简单等于某个固定硬件方案，而是要求通过冗余、监控、降级和失效安全机制，把高危失效控制到足够低的风险。预期功能安全（SOTIF）回答"系统正常但能力不够怎么办"——比如摄像头在逆光下看不见白色卡车，这不是故障，但会导致事故，需要在设计阶段就覆盖这些场景。网络安全（ISO 21434）回答"被黑客入侵怎么办"——车辆联网后，攻击面变大，需要防篡改、防远程控制。

**车企工作场景**：功能安全工程师需要为每一个安全相关功能写安全目标和安全机制——比如 AEB 误触发或漏触发都可能带来风险，必须设计传感器置信度、目标跟踪、制动阈值和驾驶员提示等多重保护。
...
**场景化问题**：为什么特斯拉曾通过 OTA 优化制动表现？软件怎么可能改变硬件体验？
...
| 方向 | OTA 内容 | 可能效果 |
|------|----------|----------|
| 制动标定 | ABS/制动控制策略优化 | 缩短特定测试条件下的制动距离 |
| 热管理 | 热泵、电池预热、空调策略优化 | 改善低温能耗和舒适性 |
| 动态控制 | 赛道模式、扭矩分配、能量回收策略 | 提升操控体验或可玩性 |
| 智驾算法 | 感知/规划模型与数据闭环升级 | 提升场景通过率和安全边界 |
