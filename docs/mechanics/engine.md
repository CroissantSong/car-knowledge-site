...
| **冷源** | 低温冷源（环境空气，约 25°C） | 冷却系统、排气管散掉的废热 |
| **卡诺效率上限** | $\eta_{Carnot} = 1 - \frac{T_{冷}}{T_{热}}$ | 理论上限约 87%，计算时温度要换算成 K |
| **实际热效率** | 奥托循环 25-35%，阿特金森 38-41% | 大部分能量被冷却液带走、从排气管喷出 |

**原理（说人话）**：热力学第二定律告诉你——热量不可能 100% 变成机械功，一定会有一部分废热必须排给冷源。卡诺循环是理论上的理想循环，热效率上限取决于热源和冷源的温度差：温差越大，效率越高。汽油机燃烧温度约 2000°C，环境温度约 25°C，换算成绝对温度后理论效率上限约 87%，但实际受限于摩擦/不完全燃烧/散热损失等，通常只有 25-35%。丰田/比亚迪的高效发动机（采用阿特金森循环+高压缩比）能到 40-41%。
...
**小测**：如果热源温度不变（燃烧室约 2000°C），但冷源温度从 300°C 降到 100°C，卡诺效率上限是变高还是变低？
...
**原理（说人话）**：轮胎能提供的总抓地力是一个「摩擦圆」——纵向加速/制动力和侧向转向力共享这个圆的半径。当你全力加速时，留给侧向转向的抓地余量会明显变少（所以急加速时打方向车不太听话）；过弯压到极限时，再踩刹车就容易冲出弯道（因为刹车吃掉了留给转向的抓地力）。
...
**油电对比 / 生活类比**：
- 油电对比：电动车重心更低、瞬时扭矩更大，干地起步响应优势明显；但整备质量通常更大，在低附着路面（冰雪）更依赖轮胎、ABS/ESC 和能量回收标定。
...
|------|-------|-------------|---------------|------|
| 奔驰 EQS | 0.20 | ~2.5 | ~0.50 | 低风阻量产轿车代表 |
| 特斯拉 Model S | 0.208 | ~2.4 | ~0.50 | 低风阻量产轿车代表 |
| 理想 MEGA | 0.215 | ~3.0 | ~0.65 | MPV 也能做到低风阻 |
| 小米 SU7 | 0.195 | ~2.2 | ~0.43 | 国产低风阻轿车代表 |
...
|------|----------|--------|--------|--------|------|
| **1. 进气** | 下行 | 开 | 关 | — | 吸入空气（汽油机可形成混合气） |
| **2. 压缩** | 上行 | 关 | 关 | — | 压缩混合气，温升压升 |
| **3. 做功** | 下行 | 关 | 关 | 点火 | 混合气燃烧膨胀，推动活塞 |
| **4. 排气** | 上行 | 关 | 开 | — | 排出燃烧废气 |

> 四个冲程中，只有**做功冲程**产生动力，其余三个冲程依靠飞轮的惯性完成。

**原理（说人话）**：发动机就像人的呼吸——吸气（进气）、憋气压缩（压缩）、燃烧膨胀发力（做功）、呼气排出（排气）。四个动作里只有「燃烧发力」那一下是活塞被推下去产生动力，其余三步全靠飞轮的惯性带动。飞轮就像陀螺，一旦转起来就会保持旋转，拖着活塞完成另外三步。

**油电对比 / 生活类比**：
- 油电对比：电动车没有冲程概念，电机可以从零转速输出高扭矩，不需要等进气、压缩和点火循环建立动力。
...
- **功率（Power）**：单位时间内做的功，更能反映持续加速和高速能力。单位：kW（千瓦）或 PS（马力）。1 PS ≈ 0.735 kW。
- **扭矩（Torque）**：发动机输出的旋转力矩，影响低速响应和带载能力。单位：N·m（牛·米）。实际加速还要看转速、齿比、车重和轮端扭矩。

> 详见 [扭矩与马力](/core-notes/torque-vs-hp)。

**油电对比 / 生活类比**：
- 油电对比：电机从 0 转就能输出高扭矩，不需要像燃油机那样等转速爬升；但功率仍等于扭矩乘转速，车速升高后电机扭矩也会进入衰减区。
- 生活类比：排量 = 肺活量（越大越能吸），压缩比 = 把弹簧压多紧（越紧弹得越远），扭矩 = 你推东西的爆发力，功率 = 你能跑多快。

**车企工作场景**：产品规划工程师在定义新车动力总成时，会根据目标用户场景（城市通勤 vs 高速巡航）匹配排量、增压方案和功率扭矩曲线。

**小测**：下列哪个参数直接描述发动机曲轴端输出的旋转力矩？
...
- **凸轮轴（Camshaft）**：一根或多根带「凸包」的轴，旋转时凸包顶开气门，凸包形状决定气门开多大、开多久。
...
```mermaid
flowchart LR
    A[化油器<br/>机械控制] --> B[电喷 EFI<br/>电子控制喷油]
    B --> C[多点电喷 MPI<br/>进气道喷射]
    B --> D[缸内直喷 GDI<br/>燃油直接喷入气缸]
    D --> E[混合喷射<br/>MPI + GDI]
```
