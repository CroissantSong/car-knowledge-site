...
const questions = computed(() => quizBank.filter(q => q.layer === 'smart-car' && q.role === 'all'))
...
> 5 道题，覆盖智驾闭环、线控制动、传感器方案、自动驾驶分级、幽灵刹车。
...
<ProgressBadge path="/quiz/smart-car" mode="checkbox" />
