...
const questions = computed(() => quizBank.filter(q => q.layer === 'traditional' && q.role === 'all'))
...
> 4 道题，覆盖动力链效率、液力变矩器、制动液压原理、制动液安全。
...
<ProgressBadge path="/quiz/traditional" mode="checkbox" />
